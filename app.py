...
df["yoy_pct"] = (
    df.groupby("series_id")["value"].pct_change(periods=12) * 100.0
)
...
work_df = work_df[work_df["series_name"].isin(selected_series)]
...
if view_mode == "Year-over-year change (%)":
    ...
    y_label = "Percent change from 12 months earlier"
else:
...
latest_levels = (
    latest_subset.set_index("series_name")["value"].reindex(selected_series)
)
latest_yoy = (
    latest_subset.set_index("series_name")["yoy_pct"].reindex(selected_series)
)
...
for i, name in enumerate(selected_series):
    ...
    if pd.isna(level):
        ...
        if pd.isna(yoy):
            ...
            delta_text = f"{yoy:+.2f}% vs. 12 months ago"
